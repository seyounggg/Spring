...
<script type="text/javascript" src="https://dapi.kakao.com/v2/maps/sdk.js?appkey=251836fae5930396e4af8398f239aee1&libraries=services"></script>
<style type="text/css">
.container{
	margin-top: 50px;
}
		...
		<div class="row">
			...
			<!-- 댓글 Vue=CURD -->
							...
							<table class="table" v-for="re in reply_data">
								...
								<tr>
									<td colspan="2" class="text-left" valign="top">
										<pre style="white-space: pre-wrap;background-color: white">{{re.msg}}</pre>
									</td>
								</tr>
				...
				<table class="table" v-if="food_detail.sessionId!=null">
					<tr>
						<td>
							<textarea rows="3" cols="55" style="float: left" :id="msg" v-model="msg"></textarea>
							<input type=button value="댓글쓰기" style="height: 65px;background-color: blue;color: white" @click="replyWrite()">
						</td>
					</tr>
				</table>
			</div>
...
new Vue({
	...
	mounted:function(){
		...
		axios.get('../food/food_detail_vue.do',{
			...
			if(this.food_detail.menu!=null){
				this.menu=this.food_detail.menu.split('원').filter(menu => menu !== '');
			}
	...
	methods:{
		...
		addScript:function(){
			...
			script.src='https://dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=251836fae5930396e4af8398f239aee1&libraries=services'
