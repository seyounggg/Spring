...
<body>
  <div class="container">
    <h1 class="text-center">VueJS(Directive:v-for(제어문))</h1>
    <div class="row">
      <table class="table">
        <tr>
          <td>
  ...
  new Vue({
	  ...
	  created:function(){
		  console.log("created:Vue객체 생성 완료")
	  ...
	  mounted:function(){
		  console.log("mounted:가상메모리에 HTML 올리기 완료")
		  // $(function()), window.onload
		  // 가상돔 / 실제돔을 비교해서 변경 => diff
		  // 가장 많이 사용되는 상태(서버에서 데이터를 읽어올 때)
	  },
	  beforeUpdate:function(){
		  console.log("beforeUpdate:상태가 변경(데이터값이 변경)되기 직전상태...")
		  // 멤버 변수 설정 : react => state, vue => data (data를 다른 메소드에서 변경시에는 this.을 작성해줘야 한다)
	  },
	  updated:function(){
		  console.log("updated:데이터변경이나 수정이 완료된 상태 -> mounted를 재호출")
	  },
	  beforeDestory:function(){
		  console.log("beforeDestory:메모리 해제 전 상태(바뀔 준비중 상태?)")
	  },
	  destoryed:function(){
